...
void loop() {
  if (Serial.available() >= 48) {
    byte buffer[48];
    Serial.readBytes(buffer, 48);  // Read all 48 bytes

    // Convert bytes to float values
    memcpy(values, buffer, 48);  // Copy raw bytes into float array

    flight_data.altitude     = values[0];
    flight_data.pos_x        = values[1];
    flight_data.pos_z        = values[2];
    flight_data.airspeed     = values[3];
    flight_data.ver_vel      = values[4];
    flight_data.pitch        = values[5];
    flight_data.roll         = values[6];
    flight_data.yaw          = values[7];
    flight_data.pitch_rate   = values[8];
    flight_data.roll_rate    = values[9];
    flight_data.yaw_rate     = values[10];
    flight_data.yaw_target   = values[11];
